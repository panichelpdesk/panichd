...
class NotificationsController extends Controller
{
	...
	public function newTicket(Ticket $ticket)
    {
		...
		if (!$ticket->hidden and Setting::grab('notices') and ($ticket->owner->panichd_notice_group_id == '0' || $ticket->owner->panichd_notice_group_id != "" ) and !in_array($ticket->owner->email, [$notification_owner->email, $ticket->agent->email])){
			$a_to[] = [
