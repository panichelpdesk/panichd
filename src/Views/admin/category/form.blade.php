<div class="row">
	...
	@if ($category->has('tags'))
		@foreach ($category->tags as $i=>$tag)		
			...
			<button type="button" id="tag_text_{{$i}}" class="btn btn-default" aria-label="Etiqueta {{$tag->name}}" title="Etiqueta '{{$tag->name}}' conté {{$tag->tickets_count}} tiquets relacionats">{{$tag->name}} ({{$tag->tickets_count}})</button>
			</div>
