...
@if (session()->has('panichd_filter_owner'))
	...
	<div class="dropdown" style="display: inline-block;">
	<button class="btn btn-light btn-sm dropdown-toggle" type="button" data-toggle="dropdown" style="border: none;"><span class="">{{ \PanicHD\PanicHD\Models\Member::find(session('panichd_filter_owner'))->name }} <span class="badge">{{ $counts['owner'] }}</span>
