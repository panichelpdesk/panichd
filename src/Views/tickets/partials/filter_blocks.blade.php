...
@else		
	...
	<button class="btn btn-light btn-category dropdown-toggle" type="button" data-toggle="dropdown" style="border: none;">{!! $category_name=="All" ? trans('panichd::lang.filter-category-all') : $category_name !!}
...
@if (session()->has('panichd_filter_owner'))
	...
	<div class="dropdown" style="display: inline-block;">
	<button class="btn btn-light dropdown-toggle" type="button" data-toggle="dropdown" style="border: none;"><span class="">{{ \PanicHD\PanicHD\Models\Member::find(session('panichd_filter_owner'))->name }} <span class="badge">{{ $counts['owner'] }}</span>
...
@else	
	...
	@if(count($filters['agent'])==1)
		<button class="btn btn-light {{ $agent_button_size }} agent-current">{{$filters['agent']{0}->name}}</button>
	@else
		@if (session('panichd_filter_agent')!="")
			<a href="{{ action('\PanicHD\PanicHD\Controllers\TicketsController@index') }}/filter/agent/remove" class="btn btn-light agent-link {{ $agent_button_size }}">{{ trans('panichd::lang.filter-agent-all') }}</a>
		...
		@foreach ($filters['agent'] as $ag)
			@if ($ag->id==session('panichd_filter_agent'))
				<button class="btn btn-light {{ $agent_button_size }} agent-current"><span>{{$ag->name}}</span> <span class="badge">{!! $counts['agent'][$ag->id] !!}</span></button>
			@else
				<a href="{{ action('\PanicHD\PanicHD\Controllers\TicketsController@index') }}/filter/agent/{{$ag->id}}" class="btn btn-light agent-link {{ $agent_button_size }}">{{$ag->name}} <span class="badge">{!! $counts['agent'][$ag->id] !!}</span></a>
			@endif			
		@endforeach	
	@endif
